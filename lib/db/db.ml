...
let parse_db_row row =
  ...
  | _ ->
    Or_error.error_s [%message "Database error: failed to parse row" (row : string array)]
;;

let or_error_of_rc rc =
  if Sqlite3.Rc.is_success rc
  then Ok ()
  else Or_error.error_string [%string "Sqlite3 error: %{Sqlite3.Rc.to_string rc}"]
;;
...
let create_table_if_not_found t () =
  Sqlite3.exec
    ...
    "CREATE TABLE IF NOT EXISTS photos (id string PRIMARY KEY, name string NOT NULL, \
     archive_path string NOT NULL UNIQUE, created_at string NOT NULL);"
  |> or_error_of_rc
;;
...
let with_db ~db_file ~f =
  ...
  let%map result =
    Monitor.try_with_join_or_error (fun () ->
        let%bind.Deferred.Or_error () = create_table_if_not_found t () |> return in
        f t)
  in
...
let insert_photo t photo =
  ...
  Sqlite3.exec
    t
    [%string
      "INSERT INTO photos (id, name, archive_path, created_at) VALUES ('%{id}', \
       '%{name}', '%{archive_path}', '%{created_at}');"]
  |> or_error_of_rc
;;
...
let lookup_photo t ~id =
  ...
  | [ row ] ->
    ...
    Some photo
  | _ ->
    Or_error.error_s [%message "Database invariant violated: id not unique" (id : string)]
;;
...
let lookup_photo_by_archive_path t ~archive_path =
  ...
  let%bind.Or_error () =
    Sqlite3.exec_not_null_no_headers
      t
      ~cb:(fun row -> results := !results @ [ row ])
      [%string "SELECT * FROM photos WHERE archive_path='%{archive_path}' LIMIT 1"]
    |> or_error_of_rc
  in
  match !results with
  | [] -> Ok None
  ...
  | _ ->
    Or_error.error_s
      [%message
        "Database invariant violated: archive_path not unique" (archive_path : string)]
;;
