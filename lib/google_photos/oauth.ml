...
let authorization_uri ~client_id =
  Uri.make ~scheme:"https" ~host:"accounts.google.com" ~path:"/o/oauth2/auth"
    ~query:
      [
        ("client_id", [ client_id ]);
  ...
  | true ->
      let%map.Deferred.Or_error refresh_token =
        try
          ...
          |> Authorize_response.t_of_yojson |> Authorize_response.refresh_token
...
let obtain_access_token t =
  ...
  | true -> (
      try
        ...
        |> Access_token_response.access_token |> Deferred.Or_error.return
      with exn ->
        Deferred.Or_error.error_s
          [%message
            "Failed to parse JSON response from authorization request"
